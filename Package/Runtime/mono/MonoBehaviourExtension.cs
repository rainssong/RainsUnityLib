...
public static class MonoBehaviourExtension
{
    ...
    /// <summary>
    /// 自动绑定，
    /// todo：绑定按名称绑定gameobject/transform,递归查找component，包含非激活
    /// 列出所有child，适配大小写
    /// </summary>
    /// <param name="mb"></param>
    ...
    /// <summary>
    /// 只能控制自身，不包含Child，最佳方案，AddCom CanvasGroup，alpha=0
    /// </summary>
    /// <param name="go"></param>
    /// <param name="visible"></param>
    public static void ChangeVisible(this GameObject go, bool visible)
    {
        Renderer re;
        if (go.TryGetComponent<Renderer>(out re))
        {
            re.enabled = visible;
        }
        CanvasRenderer cr;
        if (go.TryGetComponent<CanvasRenderer>(out cr))
        {
            cr.cull = !visible;
        }
    }
    ...
    public static void Hide(this Component mb)
    {
        mb.gameObject.SetActive(false);
    }

    public static void Show(this Component mb)
    {
